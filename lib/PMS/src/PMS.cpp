...
{
  _activeReadInterval = rate;
}

void PMS::setSleepDuration(uint32_t duration)
{
  _passiveSleepDuration = duration;
}

void PMS::setReadReadyCallback(void (*callBack)())
{
  onReadReady = callBack;
}
...
// Passive mode. In this mode sensor would send readT_RH_CO2 data to the host only for request.
...
void PMS::emptyInputStream()
{
    if (_stream->available())
    {
        if (_debugPort)
            *_debugPort << "Disgarding " << _stream->available() << " bytes from CO2 sensor" << endl;
...
void PMS::refresh()
{
  ...
  if (_mode == MODE_PASSIVE)
  {
    ...
    case SLEEPING:
      ...
      if ((labs(currentTime - _previousTime)) >= _passiveSleepDuration)
      {
        if (_debugPort)
          *_debugPort << "Time to Wakeup." << endl;
        wakeUp();
    ...
    case WAKING:
      if ((labs(currentTime - _previousTime)) >= WAKEUP_DELAY)
      {
        if (_debugPort)
        {
          *_debugPort << "Awake requesting a read." << endl;
        }
        requestRead();
        _previousTime = currentTime;
      }
      break;
    case READING:
      if ((labs(currentTime - _previousTime)) >= READ_DELAY)
      {
        if (_debugPort)
        {
          *_debugPort << "Read ready, invoking readCallback" << endl;
        }
        if (onReadReady)
          onReadReady();
        _previousTime = currentTime;
    ...
    case IDLE:
      if (_debugPort)
      {
        *_debugPort << "Data Read. Going to sleep" << endl;
      }
      _previousTime = currentTime;
      emptyInputStream();
      sleep();
    ...
    default:
      if (_debugPort)
      {
        *_debugPort << "Something is wrong" << endl;
      }
      break;
    }
  }
  ...
  {
    if ((labs(currentTime - _previousTime)) >= _activeReadInterval)
    {
      if (_debugPort)
      {
        *_debugPort << "Active Read ready. Invoking callback." << endl;
      }
      if (onReadReady)
        onReadReady();
      _previousTime = currentTime;
